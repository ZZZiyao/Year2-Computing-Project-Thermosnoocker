...
class Ball:
    ...
    def collide(self, other, restitution=1.0):
        """
        Calculate the new velocities after collision with another Ball object.
        restitution: Coefficient of restitution (default is 1.0 for elastic collisions)
        """
        if isinstance(other, Ball):
            ...
            v1_n = ((restitution * m2 * (u2_n - u1_n)) + (m1 * u1_n) + (m2 * u2_n)) / (m1 + m2)
            v2_n = ((restitution * m1 * (u1_n - u2_n)) + (m1 * u1_n) + (m2 * u2_n)) / (m1 + m2)
